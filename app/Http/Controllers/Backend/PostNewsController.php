...
class PostNewsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->breaking_news){
            $post_news->breaking_news = $request->breaking_news;
        }
    ...
    public function delete($slug){
        $post_news_delete = PostNews::where('slug', $slug)->first();
        if($post_news_delete){
            if (File::exists('backend/images/news-himages/'.$post_news_delete->news_title_image)) {
                File::delete('backend/images/news-himages/'.$post_news_delete->news_title_image);
            }
            if (File::exists('backend/images/news-himages/'.$post_news_delete->inner_news_image)) {
                File::delete('backend/images/news-himages/'.$post_news_delete->inner_news_image);
            }
        }
        $post_news_delete->delete();
        return redirect()->back()->with('add_news_success', 'Delete Successfully !');
    }
    ...
    public function approvedView(){
        ...
        $admin = Auth::guard('admin')->user();
        $user = Auth::guard('web')->user();
        $post_news_approve = PostNews::where('news_status', 1)
        ->orderBy('id_news', 'desc')->get();
        return view('backend.pages.postnews.published', compact('post_news_approve','admin','user'));
    }
    ...
    public function draftView(){
        ...
        $admin = Auth::guard('admin')->user();
        $user = Auth::guard('web')->user();
        $post_news_draft = PostNews::where('news_status', 0)
        ->orderBy('id_news', 'desc')->get();
        return view('backend.pages.postnews.draft', compact('post_news_draft','admin','user'));
    }


    // breaking_news Page
    public function breakingNewsView(){
        // $post_news_datas = PostNews::orderBy('news_id', 'desc')->paginate(100);
        $admin = Auth::guard('admin')->user();
        $user = Auth::guard('web')->user();
        $breaking_news = PostNews::where('breaking_news', 1)
        ->orderBy('id_news', 'desc')->get();
        return view('backend.pages.postnews.breaking_news', compact('breaking_news','admin','user'));
    }
